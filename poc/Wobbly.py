...
def align_pair(source1, p1, source2, p2, max_shifts, prepare, find_shifts,
    ...
    step = 1
    w1 = np.zeros((sx, sy, step))
    w1[np1x, np1y, :] = 1
    w1fft = np.fft.fftn(w1, axes=(0, 1))
    w2 = np.zeros((sx, sy, step))
    w2[np2x, np2y, :] = 1
    w2fft = np.fft.fftn(w2, axes=(0, 1))
    nrm = np.fft.ifftn(w1fft * np.conj(w2fft), axes=(0, 1))
    ...
    for a in range(start, stop, step):
        if verbose and (a - start) % 100 == 0:
            sys.stderr.write('Wobbly: alignment: slice %d / %d\n' %
                             (a, stop - start))
        i1a = i1[:, :, a - start:a - start + step]
        i2a = i2[:, :, a - start:a - start + step]
        if prepare:
            i1a = b11 * i1a + b10
            i2a = b21 * i2a + b20
        sz = i1a.shape[2]
        i1p = np.empty((sx, sy, sz))
        i2p = np.empty((sx, sy, sz))
        for j in range(sz):
            i1p[:, :, j] = np.pad(i1a[:, :, j], (pad1x, pad1y), 'constant')
            i2p[:, :, j] = np.pad(i2a[:, :, j], (pad2x, pad2y), 'constant')

        i1fft = np.fft.fftn(i1p, axes=(0, 1))
        i2fft = np.fft.fftn(i2p, axes=(0, 1))
        s1fft = np.fft.fftn(i1p * i1p, axes=(0, 1))
        s2fft = np.fft.fftn(i2p * i2p, axes=(0, 1))
        wssd = w1fft[:, :, :sz] * np.conj(s2fft) + s1fft * np.conj(
            w2fft[:, :, :sz]) - 2 * i1fft * np.conj(i2fft)
        wssd = np.fft.ifftn(wssd, axes=(0, 1))
        wssd = wssd[roilx:roiux, roily:roiuy, :]
        for j in range(sz):
            errors[a - start + j] = np.abs(wssd[:, :, j] / nrm[:, :, j])
            status[a - start + j] = MEASURED
